...
The Prism project will have an Android and iOS application that will have an intuitive UI for controlling the Hue light bulbs.  The application will interface directly with the Philip's Hue API as well as through the Web Server.
\begin{figure}[ht!]
    \centering
...
As our main software engineering process, we've chosen to use agile development. Each week will represent a single sprint, a set block of time dedicated to certain tasks aimed at iteratively adding features and polish to our product. These small sprints will allow us to remain on track to complete our project. Initially, it's unlikely that we will have a finished version of Prism at the end of each sprint, but we still plan to set goals and review the sprint for these initial weeks. Once we have our bare essentials completed for both of our mobile platforms, we should be able to complete an improved product at the end of each sprint which we can test over the course of the following sprint. This will allow us to focus on maintaining a polished project while we continue to add features.

After using Trello and Version One for team planning and delegation, we have chosen to continue using Version One. It will allow us to easily visualize where we are in the sprint by keeping track of tasks and, unlike Trello's free version, generating a burndown chart, a line graph depicting our progress vs. the progress we planned for the sprint. This system will simplify the paperwork aspect of the agile development and allow us to easily stay on track with our goals.

\subsection{Versioning, Bug Tracking, and Documentation}
For versioning, we plan on using Git with GitHub. We found that in addition to the normal public git repository, GitHub also has bug tracking, pull requests (request for peer review), and a wiki. We plan to use all of these features not only to help ourselves stay on track but to allow everyone the chance to see how the project evolves throughout the senior year.
...
\subsection{Testing}
Our main approach to testing our program will be to use it in our daily lives. To acquire third-person points of view, we will ask our friends and families to test it and give us feedback. To limit the amount of time spent manually testing our application, we also plan to use several automated testing approaches, depending on the platform.

The server will be written in Scala and we plan to use a testing framework for Scala, Specs2. In the spirit of unit testing, instead of interactions with the file system we'll use mocks with Mockito (already included in Specs2) or ScalaMock. All the code will be implemented using Test-Driven Development (TDD).

For our mobile platforms, iOS and Android, we will use unit tests to quickly verify the state of the controller and its interactions with other components in isolation. Test cases for the UI will also be created for complex UI components in addition to our manual testing.

\subsection{Team Communication and Meetings}
In order to improve team efficiency, Team Prism has agreed to use group texting, Git issue tracking, Version One, and emergency phone calls. For day-to-day communication, Team Prism will primarily use group texting. This will insure that each team member is receiving the same information as the rest of the team and will not require any second hand information. The team will also use issue tracking in Git to keep the team current on any code related issues that may arise in the development process. As mentioned above, Version One will be used for agile team planning, allowing weekly tasks and due dates to be assigned via Version One. Finally, team members will be able to call each other for any urgent issues.

In addition to varied communications throughout the week, Team Prism plans to have a brief stand-up meeting at the beginning of each week. In this meeting, each member will state what they accomplished during the previous week, what they plan to accomplish in the upcoming week, and any roadblocks that they may have run into during the week. After this stand-up meeting, each team member will be given an opportunity to express any concerns about the efficiency of the team during the previous week and offer ideas on how to improve performance as a team. This will act as Team Prism's weekly agenda meeting. Team Prism also plans to have many group meetings via Google Hangouts when the team can not meet face-to-face. For now, Team Prism plans to conduct in-person meetings on Monday and Wednesday mornings each week. This will help the team to get started early on each weekly sprint and help guarantee successful iterations as we develop the Hue project.
\clearpage
